...
int main() {
    stdio_init_all();
    printf("WS2812 White Test, using pin %d\n", WS2812_PIN);
    ...
    // Display solid white on all pixels
    while (1) {
        for (int i = 0; i < NUM_PIXELS; ++i) {
            put_pixel(pio, sm, urgb_u32(255, 255, 255));  // White (RGB)
        }
        sleep_ms(1000); // update once every second
    }

    // Optional cleanup (never reached here in infinite loop)
    pio_remove_program_and_unclaim_sm(&ws2812_program, pio, sm, offset);
}
