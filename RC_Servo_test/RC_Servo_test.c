...
#define CLOCK_DIVIDER 8.0f              // PWM clock divider
...
// Compute wrap value
uint16_t compute_wrap() {
    return (uint16_t)(SYS_CLOCK_HZ / (CLOCK_DIVIDER * SERVO_FREQ_HZ)) - 1;
}

// Convert microseconds to PWM level
uint16_t pulse_us_to_level(uint16_t us, uint16_t wrap) {
...
void servo_pwm_init(uint pin) {
    ...
    pwm_set_wrap(slice_num, compute_wrap());
...
int main() {
    ...
    float angle = 0.0f;
    float direction = STEP_DEGREES;  // forward or backward
